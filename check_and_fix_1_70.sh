...
fix_and_start_one() {
  ...
  if [ "$has_start" != "y" ]; then
    ...
    sshpass -p "$pass" ssh $SSH_OPTS root@$ip "cd /root/pnj_thantai && (git fetch origin main 2>/dev/null && git reset --hard origin/main) || (git checkout -- . 2>/dev/null; git pull origin main 2>/dev/null)" 2>/dev/null
  fi
  ...
  if [ "$result" != "ok" ]; then
    echo "  [$i] Chưa chạy, retry sau 10s..."
    sleep 10
    sshpass -p "$pass" ssh $SSH_OPTS root@$ip "cd /root/pnj_thantai && bash start_pnj.sh" 2>/dev/null || true
    sleep 15
    result=$(check_one "$ip" "$pass")
  fi
  [ "$result" = "ok" ] && echo "  [$i] OK - đã chạy" || echo "  [$i] Chưa chạy (SSH kiểm tra: ssh root@$ip)"
}
...
restart_one() {
  ...
  # Dừng cũ
  sshpass -p "$pass" ssh $SSH_OPTS root@$ip "pkill -f main.py 2>/dev/null; screen -S pnj -X quit 2>/dev/null; sleep 2" 2>/dev/null
  # Lấy code mới từ GitHub (tránh lỗi overwrite: git fetch + reset --hard)
  sshpass -p "$pass" ssh $SSH_OPTS root@$ip "cd /root/pnj_thantai && (git fetch origin main 2>/dev/null && git reset --hard origin/main) || (git checkout -- . 2>/dev/null; git pull origin main 2>/dev/null)" 2>/dev/null
  [ -f "run_16gb.sh" ] && sshpass -p "$pass" scp -o StrictHostKeyChecking=no "run_16gb.sh" "root@${ip}:/root/pnj_thantai/run_16gb.sh" 2>/dev/null
  [ -f "phones_$i.txt" ] && sshpass -p "$pass" scp -o StrictHostKeyChecking=no "phones_$i.txt" "root@${ip}:/root/pnj_thantai/phones.txt" 2>/dev/null
  sshpass -p "$pass" ssh $SSH_OPTS root@$ip "cd /root/pnj_thantai && bash start_pnj.sh" 2>/dev/null || true
...
if [ "$FORCE_ALL" = "1" ]; then
  echo ""
  echo "=== [--all] Cập nhật + restart TẤT CẢ 70 VPS (chạy song song) ==="
  for i in $(seq 1 70); do
    ip=$(get_ip "$i")
    pass=$(get_pw "$i")
    [ -z "$ip" ] && continue
